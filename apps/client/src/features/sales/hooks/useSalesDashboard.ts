...
export function useSalesDashboard() {
  ...
  async function loadLead(id: string | null) {
    ...
    setFollowUpDate(toDateTimeLocalValue(lead.nextFollowUpAt));
  }
  ...
  async function addNote(event: FormEvent<HTMLFormElement>) {
    ...
    setActiveLead(await salesApi.addNote(activeLead.id, note));
    setNote("");
  ...
  async function saveFollowUp() {
    if (!activeLead) return;
    const payloadDate = followUpDate ? new Date(followUpDate).toISOString() : null;
    const lead = await salesApi.updateFollowUp(activeLead.id, payloadDate);
    setActiveLead(lead);
    setFollowUpDate(toDateTimeLocalValue(lead.nextFollowUpAt));
  ...
  async function claimLead() {
    if (!activeLead) return;
    setActiveLead(await salesApi.claimLead(activeLead.id));
  ...
  async function requestTransfer() {
    if (!activeLead) return;
    const reason = window.prompt("Why should this lead be transferred to you?");
    if (!reason) return;
    await salesApi.requestTransfer(activeLead.id, reason);
    setNotice("Claim-transfer request submitted for admin review.");
  }
