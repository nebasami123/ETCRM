...
function ContactValue({ value, masked, kind }: { value?: string | null; masked: boolean; kind: "phone" | "email" }) {
  ...
  return <span className={`inline-flex min-w-0 items-center gap-1.5 ${masked ? "blur-[4px] select-none text-muted" : "text-foreground"}`}><Icon className="h-3 w-3 shrink-0" /> <span className="truncate">{value}</span></span>;
}
