...
export function useAdminDashboard() {
  ...
  async function resetSalesUserPassword(event: FormEvent<HTMLFormElement>) {
    event.preventDefault();
    if (!resetUserId || resetPassword.length < 8) return;
    await adminApi.resetSalesUserPassword(resetUserId, resetPassword);
    setResetPassword("");
  ...
  async function createLead(event: FormEvent<HTMLFormElement>) {
    ...
    setNotice("");
    try {
      const { assignedToId, ...payload } = newLead;
      const created = await adminApi.createLead({ ...payload, appointmentDate: newLead.appointmentDate || undefined });
      if (assignedToId) await adminApi.assignLead(created.lead.id, assignedToId);
  ...
  async function updateLeadPhase(leadId: string, phase: LeadPhase, creditedUserId?: string) {
    try {
      await adminApi.updateLeadPhase(leadId, phase, creditedUserId);
      setNotice(phase === "CLOSED_WON" ? "Lead closed won and conversion credit recorded." : "Lead phase updated.");
      await loadData();
    } catch (error) {
      setNotice(errorMessage(error, "Could not update lead phase."));
    }
  }
  ...
  function activityLabel(activity: Activity) {
    const labels = {
      ...
      LEAD_CLAIMED: "Lead claim changed",
  ...
  function activityMeta(activity: Activity) {
    ...
    try {
      const data = typeof activity.metadata === "string" ? JSON.parse(activity.metadata) as { imported?: number; skipped?: number; from?: string; to?: string; appointmentDate?: string } : activity.metadata as { imported?: number; skipped?: number; from?: string; to?: string; appointmentDate?: string };
