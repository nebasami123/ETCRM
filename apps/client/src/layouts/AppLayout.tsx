...
export function AppLayout({ children, title, subtitle }: AppLayoutProps) {
  ...
  async function submitPasswordChange(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      const result = await authClient.changePassword({ currentPassword: passwordForm.currentPassword, newPassword: passwordForm.newPassword });
      if (result.error) throw new Error(result.error.message || "Could not update password.");
      setPasswordForm({ currentPassword: "", newPassword: "", confirmPassword: "" });
      setPasswordMessage("Password updated.");
    } catch (error) {
      setPasswordError(errorMessage(error, "Could not update password."));
    } finally {
  ...
  return (
    <div className="min-h-screen bg-mist">
      <header className="sticky top-0 z-20 border-b border-line bg-white/90 shadow-[0_10px_35px_rgba(15,63,52,0.08)] backdrop-blur">
        <div className="mx-auto flex max-w-7xl items-center justify-between px-4 py-4 sm:px-6">
          ...
          <div className="flex items-center gap-3">
            ...
            <button
              ...
              className="inline-flex h-10 w-10 items-center justify-center rounded border border-line bg-white text-neutral-700 shadow-[0_6px_18px_rgba(15,63,52,0.08)] hover:bg-neutral-50"
        ...
        {showPasswordForm ? (
          <div className="border-t border-line bg-white/80 backdrop-blur">
            <form
            ...
            >
              ...
              <button
                ...
                className="rounded bg-forest px-4 py-2 text-sm font-semibold text-white shadow-[0_10px_24px_rgba(15,95,79,0.22)] hover:bg-forest/90 disabled:cursor-not-allowed disabled:opacity-60"
              >
              ...
              <button
                ...
                className="inline-flex h-10 w-10 items-center justify-center rounded border border-line bg-white text-neutral-700 shadow-[0_6px_18px_rgba(15,63,52,0.08)] hover:bg-neutral-100"
                title="Close"
              >
                <X size={18} />
              </button>
